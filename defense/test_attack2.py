#!/usr/bin/env python3
# Attack 2 — GPS spoofing (+10 days).
# Feed a spoofed GPS time to the proxy (port 25101), then send a normal,
# correctly-timestamped signed command (port 14549). L1 passes (ts is real),
# but L3 sees the signing clock and GPS disagree by 10 days → BLOCK L3-GPS-CROSS.
import socket, time, hashlib, struct, json

MAVLINK_EPOCH = 1420070400

# ── 1. Spoofed GPS time: 10 days in the future ────────────────────────────────
gps_spoof_unix = time.time() + 10 * 86400          # +10 days
...
# Prime the proxy with the spoofed GPS time first
for i in range(5):
...
print('Sent spoofed GPS (+10 days) + 10 signed packets '
