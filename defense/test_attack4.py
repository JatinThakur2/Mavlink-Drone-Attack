...
ts = time.time()                   # current timestamp — looks legitimate
ts_units = int((ts - MAVLINK_EPOCH) * 1e5)
ts_bytes = ts_units.to_bytes(6, 'little')
payload = b'\x00' * 9
header = bytes([0xFD, 9, 1, 0, 42, 1, 1, 0, 0, 0]) + payload   # seq=42 fixed (same packet every time)
...
for i in range(10):
    s.sendto(pkt, ('127.0.0.1', 14549))   # exact same packet replayed 10 times
print('Sent 10 replay packets → copy 1 PASS, copies 2-10 BLOCK L4-REPLAY')
